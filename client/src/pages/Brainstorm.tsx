...
import { Brain, Loader2, Sparkles, Target, Link2, Laugh, ChevronDown, ChevronUp } from "lucide-react";
...
export default function Brainstorm() {
  ...
  const { data: brainstorms, isLoading: isLoadingHistory, refetch } = trpc.brainstorm.list.useQuery();
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-muted-foreground text-sm">输入话题，AI 帮你发散思维</p>
        </div>
      </div>
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <Card className="club-card">
            <CardHeader className="pb-3">
              ...
              <CardDescription className="text-xs">从不同视角探讨话题</CardDescription>
            </CardHeader>
            <CardContent>
          ...
          <Card className="club-card">
            <CardHeader className="pb-3">
              ...
              <CardDescription className="text-xs">出人意料的关联</CardDescription>
            </CardHeader>
            <CardContent>
              <ul className="space-y-2">
                {currentResult.associations.map((assoc, index) => (
                  <li key={index} className="flex gap-2 text-sm">
                    <span className="text-secondary shrink-0">{index + 1}.</span>
                    <span>{assoc}</span>
                  </li>
                ))}
              </ul>
          ...
          <Card className="club-card">
            <CardHeader className="pb-3">
              ...
              <CardDescription className="text-xs">可能产生喜剧效果的方向</CardDescription>
            </CardHeader>
            <CardContent>
              <ul className="space-y-2">
        ...
        {isLoadingHistory ? (
          ...
          <Card className="club-card">
            <CardContent className="flex flex-col items-center justify-center py-8">
              ...
              <p className="text-muted-foreground text-sm">还没有头脑风暴记录</p>
            </CardContent>
          </Card>
        ) : (
          <div className="space-y-3">
            {brainstorms.map((brainstorm) => (
              <BrainstormHistoryItem key={brainstorm.id} brainstorm={brainstorm} />
            ))}
          </div>
        )}
      </div>
...
function BrainstormHistoryItem({ brainstorm }: { brainstorm: {
  id: number;
  topic: string;
  angles: string[] | null;
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <span className="text-xs text-muted-foreground">
                  {format(new Date(brainstorm.createdAt), "yyyy年MM月dd日 HH:mm", { locale: zhCN })}
                </span>
              </div>
              {isOpen ? (
