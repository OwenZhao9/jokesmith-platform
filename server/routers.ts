...
const categoryEnum = z.enum(["politics", "life", "roast", "relationship", "work", "family", "tech", "other"]);
  ...
  shows: router({
    ...
    get: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        if (!show) return null;
        const showScripts = await db.getShowScripts(input.id);
    ...
    create: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (scriptIds && scriptIds.length > 0) {
          await db.updateShowScripts(id, scriptIds);
        }
        return { id };
      }),
  ...
  inspirations: router({
    ...
    convertToScript: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const scriptId = await db.convertInspirationToScript(input.id, ctx.user.id, {
          userId: ctx.user.id,
          title: input.title,
          ...
          response_format: {
            ...
            json_schema: {
              ...
              schema: {
                type: "object",
                properties: {
                  angles: { type: "array", items: { type: "string" }, description: "切入角度" },
                  associations: { type: "array", items: { type: "string" }, description: "相关联想" },
                  punchlines: { type: "array", items: { type: "string" }, description: "笑点方向" },
                },
  ...
  ai: router({
    generateJoke: publicProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (input.usePersonalStyle && ctx.user) {
          const style = await db.getUserStyle(ctx.user.id);
          if (style) {
            const styleParts = [];
            if (style.comedyStyle) styleParts.push(`喜剧风格：${style.comedyStyle}`);
            if (style.languageHabits) styleParts.push(`语言习惯：${style.languageHabits}`);
            if (style.commonTags && style.commonTags.length > 0) styleParts.push(`常用梗：${style.commonTags.join("、")}`);
            if (style.tonePreference) styleParts.push(`语气偏好：${style.tonePreference}`);
            if (style.targetAudience) styleParts.push(`目标受众：${style.targetAudience}`);
...
4. 要有至少2-3个笑点`;
        ...
        return {
          content: contentText,
          topic: input.topic,
          usedPersonalStyle: input.usePersonalStyle && ctx.user !== null,
        };
      }),
  }),
  ...
  transcription: router({
    ...
    process: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        try {
          ...
          if ('error' in result) {
            await db.updateTranscription(input.id, ctx.user.id, { status: "failed" });
    ...
    convertToScript: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        await db.updateTranscription(input.id, ctx.user.id, { convertedScriptId: scriptId });
